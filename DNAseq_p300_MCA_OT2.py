...
if debug == 1:
	...
	spri_settle_time = 10 #min
	spri_dry_time = 10 #min
	magnet_height = 15 #mm
	incu_time1 = 5 #min
	incu_time2 = 7.5 #min
...
def calibrationcheck()
	...
	robot.pause(msg = '**********LOAD REAGENTS AND PRESS RESUME TO START PROTOCOL***********')
...
def transferspri(vol, location_final, mixvol):
	...
	p300.mix(10, mixvol, location_final)
	p300.touch_tip()
	p300.set_flow_rate(aspirate=60, dispense=60)
	p300.drop_tip()
...
def shear_clean(reaction_volume):
	...
	#1X SPRI 
	transferspri(reaction_volume * spriratio_1, magplate.wells('A1').bottom(), 2*reaction_volume - large_aspiration_extra)
	...
	p300.transfer(claseek_sample_volume, tris, magplate.wells('A1').bottom(), mix_after=(5,claseek_sample_volume - small_aspiration_extra))
	...
	p300.transfer(reaction_volume + small_aspiration_extra, magplate.wells('A1').bottom(), tempplate.wells('A1').bottom())
	magmodule.disengage()
...
def claseek(reaction_volume):
	...
	p300.transfer(claseek_end_conversion_mix_volume, end_mix, tempplate.wells('A1').bottom(), mix_after=(3, reaction_volume + claseek_end_conversion_mix_volume - small_aspiration_extra), blow_out=True)
	...
	p300.transfer(reaction_volume + small_aspiration_extra, tempplate.wells('A1').bottom(), magplate.wells('A2'), blow_out=True)
...
def spriclean(reaction_volume):
	...
	transferspri(reaction_volume * spriratio_2, magplate.wells('A2').bottom(), 2 * reaction_volume - large_aspiration_extra)
	...
	transferspri(reaction_volume * spriratio_2, magplate.wells('A3').bottom(), 2 * reaction_volume - large_aspiration_extra)
	...
	p300.transfer(final_elution_volume, tris, magplate.wells('A3').bottom(), mix_after=(5, final_elution_volume - small_aspiration_extra), blow_out=True)
...
def store():
	p300.pick_up_tip()
	p300.aspirate(final_elution_volume + large_aspiration_extra, magplate.wells('A3').bottom())
